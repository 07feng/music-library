<template>
    ...
    <div class="yin-header">
        ...
        <yin-header-nav class="yin-header-nav" :styleList="headerNavList" :active-name="activeNavName"
            @click="goPage"></yin-header-nav>
        <!-- 搜索框 -->
        <div class="header-search">
            <el-input v-model="keywords" placeholder="搜索" :prefix-icon="Search" @keyup.enter="goSearch"></el-input>
        </div>
        <yin-header-nav v-if="!token" :style-list="signList" :active-name="activeNavName"
            @click="goPage"></yin-header-nav>
        <el-dropdown class="user-wrap" v-if="token" trigger="click">
            <el-image class="user" :src="attachImageUrl(userPic)" fit="contain" />
            <template>
                <el-dropdown-menu>
                    <<el-dropdown-item v-for="(item, index) in menuList" :key="index" @click.stop="getMenuList(item.path)">{{ item.name }}</el-dropdown-item>
                </el-dropdown-menu>
            </template>
        </el-dropdown>
    </div>
